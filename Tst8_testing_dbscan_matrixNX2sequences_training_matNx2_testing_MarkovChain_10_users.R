...
#   mat=mat[!apply(mat,1,function(x) x[1]==x[2]),]
  ...
  write.table(file = "d://helloo_new4.txt",x = sprintf("user No.:\ %i\ accuracy=\ %f\ Number of matches: \ %i\ Number of mismatches: \ %i ", q, (100*j)/(j+k),j,k),row.names = F,col.names = F,append = T)
#   
