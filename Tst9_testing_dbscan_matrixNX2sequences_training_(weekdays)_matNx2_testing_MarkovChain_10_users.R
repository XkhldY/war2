...
)
{
  # q=0
  set.seed(1)
  hellooo=unique(table_user_training[table_user_training$UserID==q 
                                     ,"sequenceNo"])
...
for(w in unique(table_user$day))
{    
  table_user1=table_user[table_user$day==w,]
  ll=cbind(table_user1$Long,table_user1$Lat)
  ...
  table_user2=table_user_testing[table_user_testing$day==w,]
  if(nrow(table_user2)==0)
    next;
  ...
  cat(sprintf("user No.:\ %i\ weekday:%s\ accuracy=\ %f\ \n", q,as.character(table_user2[1,]$day), (100*j)/(j+k)))
  write.table(file = "d://helloo_newSunday.txt",x = sprintf("user No.:\ %i\ accuracy=\ %f\ Number of matches: \ %i\ Number of mismatches: \ %i", q, (100*j)/(j+k),j,k),row.names = F,col.names = F,append = T)
  #   
